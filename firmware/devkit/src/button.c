...
bool is_off = false;
static void button_ccc_config_changed_handler(const struct bt_gatt_attr *attr,
                                              uint16_t value);
...
static struct bt_uuid_128 button_uuid = BT_UUID_INIT_128(
    BT_UUID_128_ENCODE(0x23BA7924, 0x0000, 0x1000, 0x7450, 0x346EAC492E92));
static struct bt_uuid_128 button_characteristic_data_uuid = BT_UUID_INIT_128(
    BT_UUID_128_ENCODE(0x23BA7925, 0x0000, 0x1000, 0x7450, 0x346EAC492E92));
...
static struct bt_gatt_attr button_service_attr[] = {
    ...
    BT_GATT_CHARACTERISTIC(&button_characteristic_data_uuid.uuid,
                           ...
                           BT_GATT_PERM_READ, button_data_read_characteristic,
                           NULL, NULL),
    BT_GATT_CCC(button_ccc_config_changed_handler,
                BT_GATT_PERM_READ | BT_GATT_PERM_WRITE),
};

static struct bt_gatt_service button_service =
    BT_GATT_SERVICE(button_service_attr);
...
static inline void notify_press() {
  ...
  if (conn != NULL) {
    bt_gatt_notify(conn, &button_service.attrs[1], &final_button_state,
                   sizeof(final_button_state));
  }
}
...
// Note: This is accessed from main.c for LED state management
...
void check_button_level(struct k_work *work_item) {
  ...
  if (tap_count > 0 && !btn_is_pressed) {
    ...
    if (tap_count == 1 && time_since_first_tap > DOUBLE_TAP_WINDOW) {
      ...
      uint32_t time_since_second_tap =
          (current_time - btn_second_tap_time) * BUTTON_CHECK_INTERVAL;
...
//                //transition to Two_press. button could be a single or double
//                tap current_button_state = TWO_PRESS; reset_count();
//            }
//        }
//        if (state_ == 1)
//        {
...
static ssize_t button_data_read_characteristic(struct bt_conn *conn,
  ...
  return bt_gatt_attr_read(conn, attr, buf, len, offset, &final_button_state,
                           sizeof(final_button_state));
}
...
void register_button_service() { bt_gatt_service_register(&button_service); }

FSM_STATE_T get_current_button_state() { return current_button_state; }
...
void turnoff_all() {
  ...
  // maybe save something here to indicate success. next time the button is
  // pressed we should know about it
